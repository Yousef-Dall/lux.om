...
describe('PMS Stage 21G financial and portal operations', () => {
  ...
  it('scopes assets, generates preventive work orders idempotently, and converts inspection defects once', async () => {
    ...
    const run = await request(app).post('/api/pms/structured-inspections/runs').set('Authorization', `Bearer ${fixture.managerToken}`).send({ companyId: fixture.company.id, templateId: template.body.template.id, propertyId: fixture.propertyA.id, unitId: fixture.unitA.id, leaseId: fixture.leaseA.id, tenantId: fixture.tenantA.id, title: 'Move-out inspection', scheduledFor: '2026-07-20T08:00:00.000Z' }).expect(201);
    const cancellableRun = await request(app).post('/api/pms/structured-inspections/runs').set('Authorization', `Bearer ${fixture.managerToken}`).send({ companyId: fixture.company.id, templateId: template.body.template.id, propertyId: fixture.propertyA.id, unitId: fixture.unitA.id, title: 'Follow-up inspection', scheduledFor: '2026-07-21T08:00:00.000Z' }).expect(201);
    await request(app).get(`/api/pms/structured-inspections/runs?companyId=${fixture.company.id}&propertyId=${fixture.propertyA.id}&status=SCHEDULED&search=inspection&sortBy=title&direction=asc&take=1&skip=0`).set('Authorization', `Bearer ${maintenanceToken}`).expect(200).expect(({ body }) => {
      expect(body.pagination).toMatchObject({ take: 1, skip: 0, count: 1, total: 2 });
      expect(body.summary).toMatchObject({ scheduled: 2, needsAction: 0, openDefects: 0 });
      expect(body.inspections[0]).toMatchObject({ title: 'Follow-up inspection', property: { id: fixture.propertyA.id }, unit: { id: fixture.unitA.id }, template: { id: template.body.template.id, version: 1 }, _count: { results: 0, defects: 0 } });
    });
    await request(app).get(`/api/pms/structured-inspections/runs?companyId=${fixture.company.id}&propertyId=${fixture.propertyB.id}`).set('Authorization', `Bearer ${fixture.scopedToken}`).expect(403);
    await request(app).get(`/api/pms/structured-inspections/runs?companyId=${fixture.company.id}&scheduledFrom=2026-08-01&scheduledTo=2026-07-01`).set('Authorization', `Bearer ${fixture.managerToken}`).expect(400);
    await request(app).get(`/api/pms/structured-inspections/runs/${run.body.inspection.id}?companyId=${fixture.company.id}`).set('Authorization', `Bearer ${maintenanceToken}`).expect(200).expect(({ body }) => {
      expect(body.inspection.template.sections[0].items[0]).toMatchObject({ id: itemId, label: 'Cooling condition', required: true });
    });
    await request(app).post(`/api/pms/structured-inspections/runs/${cancellableRun.body.inspection.id}/cancel`).set('Authorization', `Bearer ${fixture.managerToken}`).send({ companyId: fixture.company.id, reason: 'Tenant requested a new appointment' }).expect(200).expect(({ body }) => expect(body.inspection.status).toBe('CANCELLED'));
    await request(app).post(`/api/pms/structured-inspections/runs/${cancellableRun.body.inspection.id}/cancel`).set('Authorization', `Bearer ${fixture.managerToken}`).send({ companyId: fixture.company.id, reason: 'Duplicate cancellation' }).expect(409);

    const photoTemplate = await request(app).post('/api/pms/structured-inspections/templates').set('Authorization', `Bearer ${fixture.managerToken}`).send({ companyId: fixture.company.id, propertyId: fixture.propertyA.id, name: 'Safety photo evidence', type: 'SAFETY', sections: [{ title: 'Safety', items: [{ label: 'Alarm panel', required: true, requiresPhotoOnFailure: true }] }] }).expect(201);
    const photoItemId = photoTemplate.body.template.sections[0].items[0].id as string;
    const photoRun = await request(app).post('/api/pms/structured-inspections/runs').set('Authorization', `Bearer ${fixture.managerToken}`).send({ companyId: fixture.company.id, templateId: photoTemplate.body.template.id, propertyId: fixture.propertyA.id, title: 'Safety evidence inspection' }).expect(201);
    await request(app).put(`/api/pms/structured-inspections/runs/${photoRun.body.inspection.id}/results`).set('Authorization', `Bearer ${fixture.managerToken}`).send({ companyId: fixture.company.id, results: [{ templateItemId: photoItemId, result: 'FAIL', photoUrls: [] }] }).expect(400);
    await request(app).put(`/api/pms/structured-inspections/runs/${photoRun.body.inspection.id}/results`).set('Authorization', `Bearer ${fixture.managerToken}`).send({ companyId: fixture.company.id, results: [{ templateItemId: photoItemId, result: 'PASS', photoUrls: [] }, { templateItemId: photoItemId, result: 'PASS', photoUrls: [] }] }).expect(400);

    const completed = await request(app).put(`/api/pms/structured-inspections/runs/${run.body.inspection.id}/results`).set('Authorization', `Bearer ${fixture.managerToken}`).send({ companyId: fixture.company.id, acknowledgement: { completedByName: 'Stage 21G Inspector' }, results: [{ templateItemId: itemId, result: 'FAIL', notes: 'Cooling insufficient', photoUrls: [], defect: { title: 'Repair HVAC cooling', severity: 'HIGH', photoUrls: [] } }] }).expect(200);
    expect(completed.body.inspection).toMatchObject({ status: 'NEEDS_ACTION', acknowledgement: { completedByName: 'Stage 21G Inspector' }, _count: { results: 1, defects: 1 } });
    const defectId = completed.body.inspection.defects[0].id as string;
    const otherUnit = await prisma.pmsUnit.create({ data: { companyId: fixture.company.id, propertyId: fixture.propertyA.id, unitNumber: 'A-202' } });
    const wrongUnitAsset = await request(app).post('/api/pms/assets').set('Authorization', `Bearer ${fixture.managerToken}`).send({ companyId: fixture.company.id, propertyId: fixture.propertyA.id, unitId: otherUnit.id, assetCode: 'HVAC-A-202', name: 'Other unit HVAC', category: 'HVAC', currency: 'OMR' }).expect(201);
    await request(app).post(`/api/pms/structured-inspections/defects/${defectId}/work-order`).set('Authorization', `Bearer ${fixture.managerToken}`).send({ companyId: fixture.company.id, assetId: wrongUnitAsset.body.asset.id, vendorId: fixture.vendor.id }).expect(400);
    const converted = await request(app).post(`/api/pms/structured-inspections/defects/${defectId}/work-order`).set('Authorization', `Bearer ${fixture.managerToken}`).send({ companyId: fixture.company.id, assetId: asset.body.asset.id, vendorId: fixture.vendor.id, targetDate: '2026-07-22' }).expect(201);
