...
async function mockAssetApi(page: Page, callbacks: {
  ...
  await page.route('**/api/pms/**', async (route) => {
    ...
    if (path === '/api/pms/structured-inspections/runs' && method === 'GET') return fulfillJson(route, { inspections: [{ id: 'inspection-1', title: 'HVAC safety inspection', type: 'SAFETY', status: 'NEEDS_ACTION', scheduledFor: '2026-07-20T08:00:00.000Z', completedAt: '2026-07-20T09:00:00.000Z', property: { id: propertyId, name: 'Harbour Residences' }, unit: { id: unitId, unitNumber: 'A-101' }, template: { id: 'template-1', name: 'HVAC safety', version: 1, type: 'SAFETY' }, defects: [{ id: 'defect-1', title: 'Filter replacement', description: null, severity: 'MEDIUM', status: 'OPEN', photoUrls: [], workOrderId: null, createdAt: '2026-07-20T09:00:00.000Z' }], _count: { results: 1, defects: 1, pmsDocuments: 0 } }], pagination: { take: 25, skip: 0, count: 1, total: 1 }, summary: { scheduled: 0, needsAction: 1, openDefects: 1 } });
